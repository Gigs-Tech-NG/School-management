                    ...
                    <div class="card-body">
                         ...
                         <div class="heading-layout1">
                            <div class="item-title">
                                <?php
    					include('config.php');
    					$quer=mysqli_query($connection,"SELECT fee_two FROM application WHERE level=200");
                        $total = 0;
    					while($row=mysqli_fetch_array($quer)){
                           ...
                           $income =  $total; 
    						?>
    						      <?php
    					include('config.php');
    					$quer1=mysqli_query($connection,"SELECT balance_two FROM application WHERE level=200");
                        $total = 0;
    					while($row=mysqli_fetch_array($quer1)){
    					   if(empty($row['balance_two'])){
                               echo "";
                           }else{

                           
    					   $total += $row['balance_two'];
                          
    					   }
                        }
                           $income1 =  $total; 
    						?>
                                <h3>Year2 SCHOOL FEE Payment History Total Income: #<?php echo $income;?>  (<?php
    date_default_timezone_set('Africa/Lagos');
    echo date('Y',time());?>/<?php
    date_default_timezone_set('Africa/Lagos');
    echo date('Y',time())+1;?>  Session)</h3>
                                <h3>Year2 SCHOOL FEE Payment History Total Debt: #<?php echo $income1;?>  (<?php
    date_default_timezone_set('Africa/Lagos');
    echo date('Y',time());?>/<?php
    date_default_timezone_set('Africa/Lagos');
    echo date('Y',time())+1;?>  Session)</h3>
                        ...
                        $select="SELECT * FROM application  WHERE level=200 ";
                       // $select="SELECT * FROM applicants";
                        ...
                        if(!$result){
                          ...
                          $firstname = $row['surname'];
                            $amountpaid = $row['fee_two'];
                             $balance = $row['balance_two'];
                               $paymode = $row['paymode_two'];
