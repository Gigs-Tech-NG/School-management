                        ...
                        <h1 style="font-weight: bolder;text-align:center" > <img src="images/depotter_logo.png"  style="width:; height:;margin:left:%;"><br/>DE POTTER COLLEGE OF HEALTH TECHNOLOGY
                           ...
                           <div class="heading-layout1">
                            <div class="item-title">
                                <?php
    					include('config.php');
    					$quer=mysqli_query($connection,"SELECT fee_three FROM application WHERE level=300");
                        $total = 0;
    					while($row=mysqli_fetch_array($quer)){
                           ...
                           $income =  $total; 
    						?>
    						      <?php
    					include('config.php');
    					$quer1=mysqli_query($connection,"SELECT balance_three FROM application WHERE level=300");
                        $total = 0;
    					while($row=mysqli_fetch_array($quer1)){
    					   
    					   $total += $row['balance_three'];
                          
    					   }
                           $income1 =  $total; 
    						?>
                                <h3>Year3 SCHOOL FEE Payment History Total Income: #<?php echo $income;?>  (<?php
    date_default_timezone_set('Africa/Lagos');
    echo date('Y',time());?>/<?php
    date_default_timezone_set('Africa/Lagos');
    echo date('Y',time())+1;?>  Session)</h3>
                                <h3>Year3 SCHOOL FEE Payment History Total Debt: #<?php echo $income1;?>  (<?php
    date_default_timezone_set('Africa/Lagos');
    echo date('Y',time());?>/<?php
    date_default_timezone_set('Africa/Lagos');
    echo date('Y',time())+1;?>  Session)</h3>
                        ...
                        $select="SELECT * FROM application WHERE level=300";
                       // $select="SELECT * FROM applicants";
                        ...
                        if(!$result){
                          ...
                          $firstname = $row['surname'];
                            $amountpaid = $row['fee_three'];
                             $balance = $row['balance_three'];
                               $paymode = $row['paymode_three'];
