...
$query = mysqli_query($connection,"SELECT * FROM application WHERE level = {$parent_cat} AND gender='male' ");
if(!$query){
    echo "failed";
}
...
$queried = mysqli_query($connection,"SELECT * FROM hostel WHERE id = {$room_cat}");
if(!$queried){
    echo "failed";
}
