...
include "config.php";
 ...
 $_SESSION['record']= "Name Is Not Recorded";
  echo "<script> location.href='hnd2payment.php' </script>";
 ...
 $row= mysqli_fetch_array($result);
   $schoolfeebalance= $row['balance_three'];
 $count=mysqli_num_rows($result);
 if(($schoolfeebalance > 0) OR ($schoolfeebalance == "")){
  
 $_SESSION['record']="School fee denied balance Due 300Level #".($schoolfeebalance);
  echo "<script> location.href='hnd2payment.php' </script>";
   // echo"<h3 style='color: white; font-weight: bolder; text-align: center;'>Name Is Not Recorded </h3>";
 }else{
 ...
 if(!$result){
    ...
    $update = "update application set fee_four='$minus',level='$level',year4date='$date',paymode_four='$paymode',balance_four='$balance'  where name='$name'";
