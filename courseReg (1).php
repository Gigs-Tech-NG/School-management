...
<body> 
 ...
 <?php
  ...
  //$cascore=htmlspecialchars($_POST['cascore']);
   ...
   if(empty($fullname)){
    ...
    foreach($_POST['course'] as $key => $chkcourse){ 
          include "congig.php";
  $sel=mysqli_query($connection,"SELECT * FROM  COURSESREG WHERE  course='".$chkcourse."' AND matricno = '$matricno' AND faculty='$faculty' AND dept='$dept' AND session='$session' AND level='$level' AND semester='$semester' ");             
  ...
  if($norow >=1) {
   ...
   $query=mysqli_query($connection,"INSERT INTO COURSESREG (courseRegId,discipline,fullname,matricno,session,faculty,dept,level,semester,course,examscore,cascore,regdate) 
...
<div class="row">  
   ...
   <!--- <input type="text" autocomplete="off"  name="faculty" placeholder="Search..." id="category" />
       <div class="result">
