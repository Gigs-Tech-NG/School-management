...
?>
 ...
 <?php 
  include "config.php";
   ...
   <?php 
  include "config.php";
if(isset($_GET['adgaghashqwtrvsbshjsbnamywtcbwjwfwghw'])){
    $ids =$_GET['adgaghashqwtrvsbshjsbnamywtcbwjwfwghw'];
    //$idd = $_GET[''];
    $amont=5000;
//$status=1;
$veryif="Verified";
...
require_once ('Mail/mime.php'); // PEAR Mail_Mime packge
 ...
 $to = $email_val; //enter the email address of the contact you're sending to
...
<h3 style='font-size:18px'>Dear $firstname,Your school fee payment of $amountpaid has been verified.</h3>
...
// header("Location: http://www.example.com/");
 ...
 include "config.php";
       // $update = "update applicants set (amount,status) VALUES('$amont','$status') where id='$ids'";  
        ...
        $update = "update application set schfee_verify='confirmed' where appId='$ids'";
        $checkeee=date('m/d/Y h:i:s a',time());
         $update = "update application set year1_last_check=' $checkeee' where appId='$ids'";
    ...
    if( $id  ==  $ids) {
        ...
        //echo "<script> location.href='nd1record.php'</script>";
