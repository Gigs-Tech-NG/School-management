...
<head>
	...
	<button class='glow' name="print" onclick="printPageArea('printableArea')">
                ...
                <div class="card height-auto">
                    <div class="card-body">
    					...
    					while($row=mysqli_fetch_array($quer1)){
                           ...
                           $income1 =  $total; 
    						?>
                                <h3>Year1 SCHOOL FEE Payment History Total Income: #<?php echo $income;?> (<?php
    date_default_timezone_set('Africa/Lagos');
    echo date('Y',time());?>/<?php
    date_default_timezone_set('Africa/Lagos');
    echo date('Y',time())+1;?>  Session)</h3>
                                <h3>Year1 SCHOOL FEE Payment History Total Debt: #<?php echo $income1;?> (<?php
    date_default_timezone_set('Africa/Lagos');
    echo date('Y',time());?>/<?php
    date_default_timezone_set('Africa/Lagos');
    echo date('Y',time())+1;?> Session)</h3>
                        ...
                        $sn=0;
                        $sn=$sn+1;
                        $select="SELECT * FROM application WHERE level=100 order by schfee_verify='' ";
              ...
              $fullnames = $firstname . ' ' . $middle. ' ' . $lastname;
                $dateed=$row['year1date'];
                $level= $row['level'];
             $newDate = new DateTime($row['year1date']);
